...
async function fetch1(input, init) {
...
class FetchSimplyError extends Error {
    constructor(message1, status, statusText){
        super(message1);
        this.status = status;
        this.statusText = statusText;
    }
}
const fetchSimply2 = async (url, init)=>{
    const res = await fetch(url, init);
    if (!res.ok) {
        throw new FetchSimplyError(`${res.status} '${res.statusText}' received instead of 200-299 range`, res.status, res.statusText);
    }
...
const fetchSimply1 = async (url, init)=>{
    const res = await fetch1(url, init);
    if (!res.ok) {
        throw new FetchSimplyError(`${res.status} '${res.statusText}' received instead of 200-299 range.`, res.status, res.statusText);
    }
    const contentType = res.headers.get("content-type");
...
export { fetch1 as fetchPolyfill };
export { fetchSimply2 as fetchSimply };
export { fetchSimply1 as fetchSimplyPolyfill };
